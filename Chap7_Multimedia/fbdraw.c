...
static void drawline(int fd, int start_x, int end_x, int y, ubyte r, ubyte g, ubyte b) {
    ...
    /* using `for` loop, Draw a line with points */
...
int main(int argc, char **argv) {
    ...
    /* get current frame_buffer info */
    if(ioctl(fbfd, FBIOGET_VSCREENINFO, &vinfo) < 0) {
        perror("Error reading fixed information");
        return -1;
    }

    drawpoint(fbfd, 50, 50, 255, 0, 0);     //print "RED" point
    drawpoint(fbfd, 100, 100, 0, 255, 0);   //print "GREEN" point
    drawpoint(fbfd, 150, 150, 0, 0, 255);   //print "BLUE" point
    drawline(fbfd, 0, 100, 200, 0, 255, 255);
    drawcircle(fbfd, 20, 200, 100, 255, 0, 255);
    drawface(fbfd, 0, 0, 0, 0, 255, 255, 0);

    close(fbfd);    //close frame buffer device
    return 0;
}
